...
static constexpr int    DEFAULT_VARIANT = 1;   // rcvbuf — best measured

static constexpr int DEFAULT_SIZES[]    = { 200, 4096 };
static constexpr int DEFAULT_NUM_SIZES  = 2;
...
static void pin_current_thread(int core_id) {
    // macOS affinity tags — threads with same tag share L2 cache,
    // threads with different tags prefer different physical cores.
    // core_id=0 for server, core_id=1 for client.
    thread_affinity_policy_data_t policy = { .affinity_tag = (integer_t)core_id };
...
static void apply_sockopts(int fd, int variant) {
    ...
    // TCP_NODELAY — on for all except variant 6
    if (variant != 6) {
        opt = 1;
    ...
    // Socket buffer sizes
    if (variant == 1 || variant == 3 || variant == 7 || variant == 9) {
        opt = 262144;
        setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &opt, sizeof(opt));
    }
    if (variant == 2 || variant == 3 || variant == 7) {
        opt = 262144;
...
struct server_state {
    ...
    void start(int port, int var) {
        variant = var;
        listen_fd = tcp_listen(port, variant);
        ctx = create_server_ctx(variant);
        thread = std::thread([this]() {
            if (variant == 8 || variant == 9)
                pin_current_thread(0); // server on tag 0
            int client_fd = tcp_accept(listen_fd, variant);
            SSL* ssl = SSL_new(ctx);
            SSL_set_fd(ssl, client_fd);
            apply_ssl_opts(ssl, variant);
            if (SSL_accept(ssl) <= 0) { SSL_free(ssl); close(client_fd); return; }

            std::vector<char> buf(65536 + 4);
            while (!stop.load(std::memory_order_relaxed)) {
                uint32_t net_len = 0;
                if (ssl_read_all(ssl, &net_len, 4) <= 0) break;
                uint32_t len = ntohl(net_len);
                if (len > buf.size()) break;
                if (ssl_read_all(ssl, buf.data(), (int)len) <= 0) break;
                ssl_write_all(ssl, &net_len, 4);
                ssl_write_all(ssl, buf.data(), (int)len);
            }
            SSL_free(ssl);
            close(client_fd);
        });
...
{
    int fd = tcp_connect("127.0.0.1", port, cfg.variant);
    auto* ctx = create_client_ctx(cfg.variant);
    if (cfg.variant == 8 || cfg.variant == 9)
        pin_current_thread(1); // client on tag 1

    SSL* ssl = SSL_new(ctx);
    SSL_set_fd(ssl, fd);
    apply_ssl_opts(ssl, cfg.variant);
    ...
    for (uint64_t i = 0; i < warmup_n; ++i) {
        if (ssl_write_all(ssl, &net_len, 4) <= 0) break;
        if (ssl_write_all(ssl, payload.data(), (int)payload_size) <= 0) break;
        uint32_t echo_len = 0;
        if (ssl_read_all(ssl, &echo_len, 4) <= 0) break;
        if (ssl_read_all(ssl, reply.data(), (int)payload_size) <= 0) break;
    }

    for (uint64_t i = 0; i < bench_n; ++i) {
        auto t0 = now_ns();

        int r = ssl_write_all(ssl, &net_len, 4);
        if (r <= 0) { buf.push(0); continue; }
        r = ssl_write_all(ssl, payload.data(), (int)payload_size);
        if (r <= 0) { buf.push(0); continue; }

        uint32_t echo_len = 0;
        if (ssl_read_all(ssl, &echo_len, 4) <= 0) { buf.push(0); continue; }
        if (ssl_read_all(ssl, reply.data(), (int)payload_size) <= 0) { buf.push(0); continue; }
