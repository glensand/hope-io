...
namespace hope::io {
    ...
    tls_event_loop* create_tls_event_loop() {
        ...
        public:
            tls_event_loop_impl() {
                // Always runs with both EVFILT_READ|EVFILT_WRITE registered.
            ...
            ~tls_event_loop_impl() override {
                ...
                m_tls_states.clear();
                // Copy buffers to vector before clearing set (can't modify set elements)
                    ...
                    for (int i = 0; i < nfds; ++i) {
                        ...
                        if (event.flags & EV_EOF) {
                            ...
                            if (it != m_connections.end()) {
                                auto tls_it = m_tls_states.find(fd);
                                if (tls_it != m_tls_states.end()) {
                                    handle_read((event_loop::connection&)*it, tls_it->second, cb);
                                }
                            }
                            ...
                            if (it != m_connections.end()) {
                                auto tls_it = m_tls_states.find(fd);
                                if (tls_it != m_tls_states.end()) {
                                    handle_write((event_loop::connection&)*it, tls_it->second, cb);
                                }
                            }
                        }
                    }
        ...
        private:
            void handle_accept(event_loop::callbacks& cb) {
                ...
                for (auto i = 0; i < m_cfg.max_accepts_per_tick; ++i) {
                    ...
                    // Register with kqueue for both read and write (always active)
                    struct kevent ev[2];
                    ...
                    if (ret == 1) {
                        ...
                        if (err == SSL_ERROR_WANT_READ) {
                            // Pending — retry on next read event
            ...
            void handle_read(event_loop::connection& conn, tls_per_conn& tls, event_loop::callbacks& cb) {
                ...
                std::size_t total_received = conn.buffer->consume_free([&](void* data, std::size_t size) -> std::size_t {
                    ...
                    if (err == SSL_ERROR_ZERO_RETURN) {
                        // Clean close_notify — just close, not an error.
            ...
            void handle_write(event_loop::connection& conn, tls_per_conn& tls, event_loop::callbacks& cb) {
                ...
                // EVFILT_WRITE may fire when state is read (stale after transition).
